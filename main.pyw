#This application consists of two separate .py files that are run together and communicate through text files. This is the start of the main display file.
...
#ALL IMAGES, EXCEPT "x.png", "minus.png", "plus.png", "left.png" AND "right.png" TAKEN FROM https://fireemblem.fandom.com/wiki/Fire_Emblem_Wiki
#"x.png" made by me with paint.net
#"right.png", "plus.png", "minus.png", "left.png", "right.png" made by me with Adobe Photoshop
...
def screenChange(x): # This function runs whenever the program's size is changed. It makes sure all characters fit on the screen.
  ...
  for i in range(len(deadnew)): #Used to display the dead characters. If there are too many in one row, it goes to the next row.
    while True:
      try:
        ...
        break
      except ZeroDivisionError: #If the screen is too small to fit one image, the program only allows one character in each row to prevent the program from crashing.
        row = 1
...
for i in characters: #
  if not i.endswith(".png") and not i.endswith(".jpg"): #Removes any files that aren't images.
    characters.remove(i)
...
while not done: #Pygame loop skeleton is taken from work done in class earlier in the year.
  ...
  if deadold != open(directory+'/dead.txt').readlines(): #Checks if the dead text file has been changed
    deadimg = []
    two_d = [[]]
